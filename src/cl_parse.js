...
// endless fetch/reconnect loop -- we just proceed without it, same as
// vanilla behavior for assets that are simply absent.
const knownMissingFiles = new Set();

// Reconnect after fetching a missing precache asset, so the server-info
// exchange runs again with everything in place. For a local single-player
// game, re-issuing the original "map" command is the correct/only way to
// restart the local server+client pair -- re-sending "connect <addr>"
// against a local loopback address is not a real reconnect and just
// spins retrying a network connection that was never really "remote".
function CL_ReconnectAfterFetch() {

	if ( cls.mapstring ) {

		Cbuf_AddText( cls.mapstring );

	} else if ( cls.netcon != null ) {

		Cbuf_AddText( 'connect "' + cls.netcon.address + '"\n' );
...
export function CL_ParseServerInfo() {
	...
	// before touching any other precache (everything else assumes the world
	// is already loaded).
	...
	// Lazy-load: fetch any other missing models/sounds (custom map assets
	// not bundled in a pak, e.g. a map's own monster skins or ambient
	// sounds) on demand, then reconnect so precache runs again with
	// everything in place.
	{

		const missing = [];

		for ( let i = 2; i < nummodels; i ++ ) {

			// Names starting with '*' are inline brush submodels (part of
			// the already-loaded world BSP), not standalone files to fetch.
			if ( model_precache[ i ].charAt( 0 ) === '*' )
				continue;

			if ( knownMissingFiles.has( model_precache[ i ] ) )
				continue;

			if ( COM_FindFile( model_precache[ i ] ) === null )
				missing.push( model_precache[ i ] );

		}

		for ( let i = 1; i < numsounds; i ++ ) {

			const path = 'sound/' + sound_precache[ i ];
			if ( knownMissingFiles.has( path ) )
				continue;

			if ( COM_FindFile( path ) === null )
				missing.push( path );
		...
		if ( missing.length > 0 ) {
			...
			Con_Printf( 'Fetching %d missing file(s)...\n', missing.length );

			Promise.all( missing.map( ( f ) => COM_EnsureFile( f ) ) ).then( ( results ) => {
				...
				for ( let i = 0; i < missing.length; i ++ ) {
					...
					if ( ! results[ i ] ) {
						...
						Con_Printf( 'Couldn\'t fetch %s\n', missing[ i ] );
						knownMissingFiles.add( missing[ i ] );
